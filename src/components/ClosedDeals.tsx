    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h3 className="text-base md:text-lg font-bold uppercase tracking-widest text-brand-zinc-100 flex items-center gap-3">
          <BadgeDollarSign size={18} className="text-brand-primary w-5 h-5 md:w-6 md:h-6" />
          Invoice_Archive
        </h3>
      ...
      <div className="glass-card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-[10px] md:text-xs text-left font-mono min-w-200 md:min-w-225">
            <thead className="bg-brand-card/50 text-brand-zinc-500 font-bold uppercase tracking-widest border-b border-brand-border">
              <tr>
                <th className="px-4 md:px-6 py-3 md:py-4 truncate max-w-25">Entity_ID</th>
                ...
                closedLeads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-brand-primary/5 transition-colors group">
                    ...
                    <td className="px-4 md:px-6 py-3 md:py-4">
                      <div className="font-bold text-brand-zinc-100 mb-0.5 truncate max-w-37.5 md:max-w-none">{lead.companyName}</div>
                      <div className="text-[9px] md:text-[10px] text-brand-zinc-500 truncate max-w-37.5 md:max-w-none">{lead.contactName || "N/A"}</div>
                    </td>
                    <td className="px-4 md:px-6 py-3 md:py-4">
                    ...
                    <td className="px-4 md:px-6 py-3 md:py-4">
                      {lead.status === 'closed' && lead.proposal && (
                        ...
                        >
                          <Download size={12} className="w-3 h-3 md:w-4 md:h-4" />
                        </button>
                      )}
                    </td>
                  </tr>
